use issue61_eval::{
    ...
    GateVerdict, MetricKind, PairedBlock, ALPHA, MAX_REL_HALFWIDTH, MIN_BLOCKS,
};
...
fn a_tight_low_variance_cell_passes() {
    ...
    assert!(cell.passes);
    assert!(cell.rel_halfwidth <= MAX_REL_HALFWIDTH);
...
fn underpowered_cells_force_fix_measurement() {
    ...
    let report = evaluate(vec![cell], Some(passed_calibration()), true);
...
#[test]
fn missing_calibration_forces_fix_measurement() {
    ...
    let report = evaluate(vec![cell], None, true);
...
fn failed_equivalence_still_forces_fix_measurement() {
    ...
    let report = evaluate(vec![cell], Some(passed_calibration()), false);
...
fn exit_code_is_nonzero_for_fix_measurement() {
    ...
    let report = evaluate(vec![stable_cell()], None, true);
...
fn keep_requires_every_preregistered_condition() {
    ...
    let calibration = passed_calibration();

    // When: the complete gate is evaluated.
    let report = evaluate(vec![stable_cell()], Some(calibration), true);
