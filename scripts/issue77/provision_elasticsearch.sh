#!/usr/bin/env bash
# Issue #77 (Infra E3): provision a fresh single-node Elasticsearch
...
BASE="http://localhost:${I77_ES_PORT}"
BUILD_TIMEOUT="${I77_BUILD_TIMEOUT_SECONDS:-3600}"

echo "==> removing any existing container $I77_ES_CONTAINER"
docker rm -f "$I77_ES_CONTAINER" >/dev/null 2>&1 || true

echo "==> starting $I77_ES_CONTAINER ($I77_ES_IMAGE)"
docker run -d \
  --name "$I77_ES_CONTAINER" \
  --cpus="$I77_CPUS" \
  ...
  -e "ES_JAVA_OPTS=-Xms${I77_JVM_HEAP} -Xmx${I77_JVM_HEAP}" \
  "$I77_ES_IMAGE" >/dev/null

echo "==> waiting for Elasticsearch cluster health (timeout ${I77_READINESS_TIMEOUT_SECONDS}s)"
READY=""
STATUS=""
...
if [[ -z "$READY" ]]; then
  echo "FATAL: Elasticsearch did not become ready within ${I77_READINESS_TIMEOUT_SECONDS}s" >&2
  docker logs "$I77_ES_CONTAINER" >&2
  exit 3
fi
...
INDEX_BYTES="$(docker exec "$I77_ES_CONTAINER" du -sb "$DATA_DIR" | awk '{print $1}')"
if [[ -z "$INDEX_BYTES" ]]; then
  echo "FATAL: could not measure index size at $DATA_DIR" >&2
  exit 5
fi

echo "PROVISION_OK container=$I77_ES_CONTAINER docs=$ACTUAL_DOCS index_bytes=$INDEX_BYTES"
