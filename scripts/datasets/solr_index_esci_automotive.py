...
schema + bulk-index the real ESCI automotive-parts slice into Apache
Solr, mirroring solr_index_esci_electronics.py's own pattern exactly
for a fair, independent baseline.

Usage: python3 scripts/datasets/solr_index_esci_automotive.py [core_url]
"""
import json
import sys
import time
...
SOLR_URL = sys.argv[1] if len(sys.argv) > 1 else "http://localhost:8983/solr/esci_automotive_bench"
CATALOG = Path(__file__).resolve().parents[2] / "dataset_cache" / "esci_automotive" / "esci_automotive_products.jsonl"
BATCH_SIZE = 500
