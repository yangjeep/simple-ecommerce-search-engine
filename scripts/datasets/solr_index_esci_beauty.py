#!/usr/bin/env python3
"""Issue #35 (docs/experiments/ISSUE35_ESCI_BEAUTY_PROTOCOL.md): schema
+ bulk-index the real ESCI beauty/personal-care slice into Apache Solr,
mirroring solr_index_esci_electronics.py / solr_index_esci_automotive.py
exactly for a fair, independent baseline.

Usage: python3 scripts/datasets/solr_index_esci_beauty.py [core_url]
"""
import json
import sys
import time
...
SOLR_URL = sys.argv[1] if len(sys.argv) > 1 else "http://localhost:8983/solr/esci_beauty_bench"
CATALOG = Path(__file__).resolve().parents[2] / "dataset_cache" / "esci_beauty" / "esci_beauty_products.jsonl"
BATCH_SIZE = 500
