#!/usr/bin/env bash
# Issue #62 (Infra E2): provision a frozen, single-node OpenSearch container
# under the E2 resource limits, stream-bulk-index a WANDS-format catalog, and
# verify the corpus indexed cleanly (exact doc count match) before reporting
# on-disk index footprint.
#
# OpenSearch is an Elasticsearch fork with a compatible core REST API at the
# version used here, so this script's shape deliberately mirrors
# provision_elasticsearch.sh. It measures physical footprint (disk bytes /
# RAM behavior) at increasing catalog scale (100k-5M rows) -- it is NOT a
# relevance-tuning or query-latency script: the mapping below is a
# reasonable production-typical ecommerce mapping, not a tuned one.
#
# Security is disabled via DISABLE_SECURITY_PLUGIN=true (verified working
# against opensearchproject/opensearch:2.19.0 -- the sibling
# `plugins.security.disabled=true` env var conflicts with it and the
# container fails to start if both are set). This is a local single-tenant
# benchmark container, not a production deployment, so no auth is a
# deliberate simplification, not an oversight.
#
# This script is intentionally self-contained (its Python bulk indexer is
# inlined below, not shared with provision_elasticsearch.sh) so the two
# engines' provisioning scripts can be modified independently.
#
# Usage:
#   bash scripts/issue62/provision_opensearch.sh <catalog_path> <expected_docs>
#
...
BASE="http://localhost:${I62_OPENSEARCH_PORT}"
BUILD_TIMEOUT="${I62_BUILD_TIMEOUT_SECONDS:-3600}"

# --- 1. fresh container ------------------------------------------------------
echo "==> removing any existing container $I62_OPENSEARCH_CONTAINER"
docker rm -f "$I62_OPENSEARCH_CONTAINER" >/dev/null 2>&1 || true

echo "==> starting $I62_OPENSEARCH_CONTAINER ($I62_OPENSEARCH_IMAGE)"
docker run -d \
  --name "$I62_OPENSEARCH_CONTAINER" \
  --cpus="$I62_CPUS" \
  --cpuset-cpus="$I62_CPUSET" \
  --memory="$I62_MEMORY" \
  --memory-swap="$I62_MEMORY_SWAP" \
  -p "${I62_OPENSEARCH_PORT}:9200" \
  -e "discovery.type=single-node" \
  -e "DISABLE_SECURITY_PLUGIN=true" \
  -e "OPENSEARCH_JAVA_OPTS=-Xms${I62_JVM_HEAP} -Xmx${I62_JVM_HEAP}" \
  "$I62_OPENSEARCH_IMAGE" >/dev/null

# --- 2. wait for readiness ---------------------------------------------------
echo "==> waiting for OpenSearch cluster health (timeout ${I62_READINESS_TIMEOUT_SECONDS}s)"
READY=""
STATUS=""
...
if [[ -z "$READY" ]]; then
  echo "FATAL: OpenSearch did not become ready within ${I62_READINESS_TIMEOUT_SECONDS}s" >&2
  docker logs "$I62_OPENSEARCH_CONTAINER" >&2
  exit 3
fi
...
INDEX_BYTES="$(docker exec "$I62_OPENSEARCH_CONTAINER" du -sb "$DATA_DIR" | awk '{print $1}')"
if [[ -z "$INDEX_BYTES" ]]; then
  echo "FATAL: could not measure index size at $DATA_DIR" >&2
  exit 5
fi

echo "PROVISION_OK container=$I62_OPENSEARCH_CONTAINER docs=$ACTUAL_DOCS index_bytes=$INDEX_BYTES"
