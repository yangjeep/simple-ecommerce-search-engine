...
- Phase 9 originally found native's ranking pass **slower** than Solr-restricted (0.42x–0.60x) on real, structural-routed WANDS traffic once realistic candidate-set sizes were used. Localizing that cost (Issue #55) found two fixable implementation defects, not a fundamental limit; once fixed, the identical real comparison flips to native **4.6x–8.2x faster**, reversing the earlier finding (`docs/decisions/PHASE9_DECISION.md`'s dated addendum). That gain does not hold unconditionally: a whole-workload follow-up found one of the two fixes **regresses ~1.7x–2.3x** on queries with no residual lexical tokens to score, root-caused to an unconditional per-candidate hashmap lookup the fix added ahead of the (now-bypassed) zero-work short-circuit the old code had for exactly that case — amplified to full-catalog scale by two real, zero-constraint WANDS queries (`docs/decisions/ISSUE55_WHOLE_WORKLOAD_DECISION.md`).
