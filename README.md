...
- Phase 9 originally found native's ranking pass **slower** than Solr-restricted (0.42x–0.60x) on real, structural-routed WANDS traffic once realistic candidate-set sizes were used. Localizing that cost (Issue #55) found two fixable implementation defects, not a fundamental limit; once fixed, the identical real comparison flips to native **4.6x–8.2x faster**, reversing the earlier finding (`docs/decisions/PHASE9_DECISION.md`'s dated addendum). A whole-workload follow-up found one of the two fixes **regressed ~1.7x–2.3x** on queries with no residual lexical tokens to score (an unconditional per-candidate hashmap lookup ahead of a now-bypassed zero-work short-circuit); that regression is now fixed and validated, with a small **additional** 2.5–8.2% gain from the same reorder and no correctness change (`docs/decisions/ISSUE55_EMPTY_RESIDUAL_FIX_DECISION.md`). The remaining whole-workload gap traces to catalog-scan cost outside either fix's scope, named as an open follow-up rather than closed.
